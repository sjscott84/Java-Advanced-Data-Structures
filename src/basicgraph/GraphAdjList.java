...
public class GraphAdjList extends Graph {
	 ...
	 */		
	 public List<Integer> getDistance2(int v) {
		 ...
		 List<Integer> oneStepNeighbors = getNeighbors(v);
		 for(int i = 0; i < oneStepNeighbors.size(); i++){
			 List<Integer> neighbor = getNeighbors(oneStepNeighbors.get(i));
			 for(int j = 0; j < neighbor.size(); j++){
				 twoHop.add(neighbor.get(j));
			 }
		 }
		 return twoHop;
	}
